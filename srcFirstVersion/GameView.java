...
public class GameView extends JPanel implements Observer 
{
	 ...
	 */
	private void updateBallPosition(GameEvent gameEvent)
	{
		int dx = gameEvent.getDx();
		int dy = gameEvent.getDy();
		ball.move(dx, dy);
		repaint();
	}
	 ...
	 */
	private void updatePaddlePosition(GameEvent gameEvent)
	{
		
		int dx = gameEvent.getDx();
		int dy = gameEvent.getDy();
		int paddleIndex = gameEvent.getPaddleIndex();
		paddles[paddleIndex].move(dx, dy);
		repaint();
	}
	...
	public void update(Observable observable, Object object) 
	{
		if(object instanceof GameEvent)
		{
			...
			if(gameEvent.getEventType() == GameEvent.GAME_EVENT_BALL)
			{
				updateBallPosition(gameEvent);
			}
			else if(gameEvent.getEventType() == GameEvent.GAME_EVENT_PADDLE)
			{
				updatePaddlePosition(gameEvent);
			}
			else if(gameEvent.getEventType() == GameEvent.GAME_EVENT_GAME_CHANGER)
			{
				
			}
			else if(gameEvent.getEventType() == GameEvent.GAME_EVENT_GAMEOVER)								
			{
				controller.stop();
				ball.setColor(Color.black);
				
				winnerTextLabel.setText("Winner is : " + gameEvent.getWinner() + "!");
				// Create highscore panel
				highScorePanel = new HighScorePanel(gameEvent.getPlayers());
			...
			else if (gameEvent.getEventType() == GameEvent.GAME_EVENT_POINTS)
			{
				updateScoreFields(gameEvent.getPlayerIndex(), gameEvent.getPoints());
			}
			else if (gameEvent.getEventType() == GameEvent.GAME_EVENT_PLAYERDEAD)
			{
				deActivatePaddle(gameEvent.getPlayerIndex());
			}
		}
