 ...
 * necessary algorithms for movement and physics within the game.
 * @author Grupp 4
 * @version 2.0
 * @version 2012-02-13
 *
...
public class Controller implements ActionListener
{
	...
	private boolean isPaddlesActive;
	...
	/**
	 * Updates the AI if there is a player that is activated as an AI
	 */
	private void updateAI() 
	{
		...
		if ( players!= null)
		{
			...
			{
				if(players[i].isAI())
				ai.calculateNextAiMove(gameView.getPaddles()[i] , gameView.getBall(), i);
			}
		}
	}
	...
	private void horizontalPaddleUpdate()
	{
		...
		for(int paddleIndex = 2; paddleIndex < 4; paddleIndex++)
		{
			...
			double dynamicSpeedConstant = (paddles[paddleIndex].getWidth() / PADDLE_BASE_SPEED_CORRECTION);
			...
			{
				gameModel.setPaddleAction(paddleIndex,(int)(paddleSpeed[paddleIndex]*dynamicSpeedConstant) , 0);
			}
	...
	private void verticalPaddleUpdate()
	{
		...
		for(int paddleIndex = 0; paddleIndex < 2; paddleIndex++)
		{
			...
			double dynamicSpeedConstant =(paddles[paddleIndex].getHeight() / PADDLE_BASE_SPEED_CORRECTION);
			...
			else
			{
				gameModel.setPaddleAction(paddleIndex,
								0, 
								(int)(paddleSpeed[paddleIndex]*dynamicSpeedConstant));
			}			
			if (paddleSpeed[paddleIndex]>0)
			{
				paddleSpeed[paddleIndex]--;
	...
	private void checkChanger()
	{
		...
		{
			// Checks corners and center for collision detect
			if (gameChangerIcon.contains(ball.getCenterX(), ball.getCenterY())||
				(ball.contains(gameChangerIcon.getX(), gameChangerIcon.getY()))||
				(ball.contains(gameChangerIcon.getX(), gameChangerIcon.getY() + gameChangerIcon.getHeight()))||
				(ball.contains(gameChangerIcon.getX() + gameChangerIcon.getWidth(), gameChangerIcon.getY()))||
				(ball.contains(gameChangerIcon.getX() + gameChangerIcon.getWidth(), gameChangerIcon.getY() + gameChangerIcon.getHeight())))	
			{
					gameChangerHandler.deactivateGameChanger();
					changerPeriod=0;
